...
import { getContent, ResearchFrontmatter, ContentResponse } from '../../lib/content';

const ResearchSection: React.FC = () => {
  const [content, setContent] = useState<ContentResponse | null>(null);

  useEffect(() => {
    getContent('research').then((data) => setContent(data));
  }, []);

  if (!content) return null;

  const { frontmatter } = content as { frontmatter: ResearchFrontmatter };

  return (
    <section id="research" className="relative min-h-screen py-24">
